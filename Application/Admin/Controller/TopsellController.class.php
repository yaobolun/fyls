...
/*跳转*/
	...
	public function report(){
		if($_SESSION['id']=='')
    	{
			...
			$news=M('news');//实例化数据表
			$count=$news->where("top_id=2")->count();// 查询满足要求的总记录数
			...
			$arr=$news->where($map)->join("report_categories on news.n_id=report_categories.id")->field('news.title,news.abstract,news.table,news.figures,news.U_price,news.time,news.published,report_categories.type,news.type_id,news.pages,news.u_price2,news.top_id')->order('news.time desc')->limit($Page->firstRow.','.$Page->listRows)->select();
	...
	public function report_add(){
		...
		if(!empty($_POST['sub'])){
			...
			$query=$news->add($map);
			if($query>0){
				echo $this->jump('添加成功','Topsell/report');
			}
			else{
				echo $this->jump('添加失败','Topsell/report_add');
			}
		}else{
			...
			$report2=$report->where("type_id!=0")->select();
	...
	public function report_del()
	{
		...
		if(!empty($_GET['id'])){
			...
			if($val>0)
			{
				echo $this->jump("删除成功","Topsell/report");
			}else 
				{
				echo $this->jump("删除失败","Topsell/report");
			}		

		}}
	}
	//数据修改
	public function report_mod(){
		if($_SESSION['id']=='')
    	{
    		echo $this->jump('请登录',"Index/login");
    	}else {
		...
		if (!empty($_POST['sub'])) {
			...
			if($val)
			{
				echo $this->jump("修改成功","Topsell/report");
			}else {
				echo $this->jump("修改失败","Topsell/report");
			}
		}
  ...
  public function upload($data){
		...
		// $this -> excel($filename);
	  	if (!empty($_POST['sub'])) {
			...
			if ($query) {
				...
				//echo $a;die;
				$this->excel($filename);
				echo $this->jump("上传成功","Topsell/report");
			}
			else{
				echo $this->jump("上传失败","Topsell/report");
			}
	  	}
	  	else{
	  		$this->display();
	...
	public function excel_out(){
		$m =M("news");
		$result = $m->where("top_id=2") ->select();
