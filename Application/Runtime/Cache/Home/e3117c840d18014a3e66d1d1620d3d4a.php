...
<title><?php echo ($mbx["type"]); ?>1</title>
<!-- <script>
...
.cp_list  .dl1,.dl2,.dl3,.dl5,.dl6,.dl7,.dl9,.dl10,.dl11{
	width:233px;
...
<div class='product_content'>
	...
	<div class='pc_right'>
           ...
           <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><a href='/zjtr/Home/Mall/details?active=<?php echo ($n); ?>&id=<?php echo ($data["id"]); ?>'>
               <dl class='dl<?php echo ($a++); ?>'>
                   <dt><img src='/zjtr/Public/<?php echo ($data["diagram"]); ?>'></dt>
                   <dd style='color:#2695D9;margin-top:10px'>￥<?php echo ($data["price"]); ?>/<?php echo ($data["danwei"]); ?></dd>
                   <dd style='margin:5px 0 10px 0'><?php echo ($data["title"]); ?></dd>
               </dl>
             </a><?php endforeach; endif; else: echo "" ;endif; ?>    
              
           </div>
           
           <div class='pcr_page'><span class='sy' style='margin-right:1%'>首页</span><?php echo ($page); ?><!-- <span class='syy'>上一页</span><span class='sz'>1</span><span class='xyy'>下一页</span> --> 共<?php echo ($count); ?>页 到 <input type='text' value='1' class='pcr_text' id='yema' onchange='fuzhi()'>页<a class='pcr_sub' id='dianjia' onclick='fuzhi()'>确定</a></div>
	</div>
...
function   fuzhi(){
	...
	if(id){
		$("#dianjia").attr("href","/zjtr/Home/Mall/<?php echo ($ff); ?>/active/<?php echo ($n); ?>/id/<?php echo ($id); ?>/p/"+num+".html");
	}else{
		$("#dianjia").attr("href","/zjtr/Home/Mall/<?php echo ($ff); ?>/active/<?php echo ($n); ?>/p/"+num+".html");
	}
