 ...
 <div class="rt_content">
      ...
      <table class="table">
       ...
       <?php if(is_array($arr)): $i = 0; $__LIST__ = $arr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$arr): $mod = ($i % 2 );++$i;?><tr>
        ...
        <?php if($arr["flag"] == 0): ?><td class="center">审批中</td>
        <?php elseif($arr["flag"] == 1): ?>
          <td style="color:green;" class="center">审批中</td>
        <?php elseif($arr["flag"] == 2): ?>
          <td style="color:green;" class="center">收货中</td>
        <?php elseif($arr["flag"] == 3): ?>
          <td style="color:green;" class="center">等待发货中</td>
        <?php elseif($arr["flag"] == 4): ?>
          <td style="color:green;" class="center">已发货</td>
        <?php elseif($arr["flag"] == 6): ?>
          <td style="color:red;" class="center">被拒绝</td><?php endif; ?>

        <td class="center">
        <?php if($arr["flag"] == 1): ?><a onclick="nono()" title="修改" class="link_icon">&#101;</a>
          <a onclick="nono()" title="删除" class="link_icon">&#100;</a>
        <?php elseif($arr["flag"] == 2): ?>
          <a onclick="nono()" title="修改" class="link_icon">&#101;</a>
          <a onclick="nono()" title="删除" class="link_icon">&#100;</a>
        <?php elseif($arr["flag"] == 3): ?>
          <a onclick="nono()" title="修改" class="link_icon">&#101;</a>
          <a onclick="nono()" title="删除" class="link_icon">&#100;</a>
        <?php elseif($arr["flag"] == 4): ?>
          <a onclick="nono()" title="修改" class="link_icon">&#101;</a>
          <a onclick="nono()" title="删除" class="link_icon">&#100;</a>
        <?php elseif($arr["flag"] == 5): ?>
          <a onclick="nono()" title="修改" class="link_icon">&#101;</a>
          <a onclick="nono()" title="删除" class="link_icon">&#100;</a>
        <?php elseif($arr["flag"] == 0): ?>
          <a onclick="expre_modd(<?php echo ($arr["id"]); ?>)" title="修改" class="link_icon">&#101;</a>
          <a onclick="expre_del(<?php echo ($arr["id"]); ?>)" title="删除" class="link_icon">&#100;</a>
        <?php elseif($arr["flag"] == 6): ?>
          <a onclick="expre_modd(<?php echo ($arr["id"]); ?>)" title="修改" class="link_icon">&#101;</a>
          <a onclick="expre_del(<?php echo ($arr["id"]); ?>)" title="删除" class="link_icon">&#100;</a><?php endif; ?>
        </td>
...
function expre_modd($id){
    location.href = "<?php echo C('HOME_PATH');?>"+'/Expre/expre_modd?id='+($id);
}
function nono()
{
  alert('无法操作哦！');
