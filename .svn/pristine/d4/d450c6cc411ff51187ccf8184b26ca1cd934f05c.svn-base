 ...
 <div class="rt_content">
     ...
     <form action="" method="post"  enctype="multipart/form-data">
      ...
      <li>
        ...
        <?php if(is_array($rc)): $i = 0; $__LIST__ = $rc;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rc): $mod = ($i % 2 );++$i;?><option value='<?php echo ($rc["id"]); ?>' <?php if(($rc["id"]) == $sel["n_type"]): ?>selected="selected"<?php endif; ?>><?php echo ($rc["type"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        </select>
       </li>
       <li>
      ...
      <li>
        ...
        <?php if(is_array($c)): $i = 0; $__LIST__ = $c;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$c): $mod = ($i % 2 );++$i;?><input type="checkbox"  name="colour[]" value='<?php echo ($c["col_id"]); ?>' <?php if(in_array(($c["col_id"]), is_array($sel["colour"])?$sel["colour"]:explode(',',$sel["colour"]))): ?>checked<?php endif; ?>/><?php echo ($c["col_name"]); ?>&nbsp;&nbsp;&nbsp;&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?>
       </li>
       <li>
        <span class="item_name" style="width:120px;">价格：</span>
         <input type="text" class="textbox textbox_295" value="<?php echo ($sel["price"]); ?>" name="price" />
         <span class="item_name" style="">单位：</span>
         <input type="text" class="textbox textbox_250" value="<?php echo ($sel["danwei"]); ?>" name="danwei" />
       </li>
       <li>
        <span class="item_name" style="width:120px;">数量：</span>
         <input type="text" class="textbox textbox_295" value="<?php echo ($sel["number"]); ?>"  placeholder="数量..." name="number" />
       </li>
       <li>
        <span class="item_name" style="width:120px;">产品图片：</span>
          <?php if(is_array($img)): $i = 0; $__LIST__ = $img;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$imgs): $mod = ($i % 2 );++$i;?><input type='hidden' name="old_img<?php echo ($imgs['bh']); ?>" class="oldimg<?php echo ($imgs['bh']); ?>" value="<?php echo ($imgs['img']); ?>"><?php endforeach; endif; else: echo "" ;endif; ?>
         <span class='image_container' name='img'>
         <?php if(is_array($img)): $i = 0; $__LIST__ = $img;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$diagram): $mod = ($i % 2 );++$i;?><img class="fileimg_<?php echo ($diagram['bh']); ?>"  src="/zjtr/Public/<?php echo ($diagram['img']); ?>" height="100" width="100" numb="<?php echo ($diagram['bh']); ?>" onclick='del(this)'> &nbsp;<?php endforeach; endif; else: echo "" ;endif; ?>
         </span>
         <span class='upload_container'> 
              
         </span>
         <img id='addfiles' src='/zjtr/Public/images/z_add.png' num='<?php echo ($num); ?>' width='100' height="100" onclick="addfile(this)">
         
       </li>
       <li>
        <span class="item_name" style="width:120px;">税：</span>
         <input type="radio"  value="1" name="tax" <?php if($sel["tax"] == 1): ?>checked<?php endif; ?>/>含税
         <input type="radio"  value="2" name="tax" <?php if($sel["tax"] == 2): ?>checked<?php endif; ?>/>不含税
       </li>
       <li>
