...
.baocun{
    width:75px;text-align:center;background:#008CD6;color:#fff;padding:5px;font-size:16px;display: block;float:left;
}
*{margin:0;padding:0;list-style-type:none;}
a,img{border:0;}
body{font:12px/180% Arial, Helvetica, sans-serif, "新宋体";}
...
<div class='product_content'>
	...
	<div class='pc_right' id='xx'>
           ...
           <div class='pcr_right' style='float:left'>
                 <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><p>收货地址：<?php echo ($data["a_address"]); ?>
                    	<span style='float:right;margin-right:20px'><a href='/zjtr/Home/My/upaddress?id=<?php echo ($data["a_id"]); ?>'>修改</a></span>
                    </p><?php endforeach; endif; else: echo "" ;endif; ?>   
                 <div>
                      <a href='#' class='xgxx' onclick='xxk1()' >增加地址</a>
         
                 </div>
           </div>
	</div>
	<div class='pc_right' hidden id='bao'>
	  <form action='/zjtr/Home/My/addaddress' method='post' enctype="multipart/form-data">

           <div class='pcr_right'  style='float:left'>
                 <p>收货地址：<input type='text' name='a_address' ></p>
                 
                 <div>
                     <!--  <a href='#' class='xgxx'>增加地址</a> -->
