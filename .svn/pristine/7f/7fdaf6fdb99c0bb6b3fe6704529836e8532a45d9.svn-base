	...
	<div class='pc_right' id='xx'>
           ...
           <div class='pcr_right' style='float:left'>
                 <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><p>收货地址：<?php echo ($data["a_address"]); ?>
                    	<span style='float:right;margin-right:20px'><a href='/zjtr/Mobile/My/upaddress?id=<?php echo ($data["a_id"]); ?>'>修改</a></span>
                    </p><?php endforeach; endif; else: echo "" ;endif; ?> 
                  
                 <div>
                      <a href='#' class='baocun' onclick='xxk1()' >增加地址</a>
         
                 </div>
           </div>
	</div>
	<div class='pc_right' hidden id='bao'>
	  <form action='/zjtr/Mobile/My/addaddress' method='post' enctype="multipart/form-data">

           <div class='pcr_right'  style='float:left'>
                 <p>收货地址：<input type='text' name='a_address' style='width:70%;border:1px solid #000'></p>
